	...
	if (0) { //ccteng {
		...
		if(exeNameTCHAR && exeName)
		{
			...
			*/
			...
			retSize = 0;//ccteng GetModuleFileName(NULL, exeNameTCHAR, DIR_BUFFER_LEN);
...
unsigned long PVRShellInit::OsGetTime()
{
	...
	return (unsigned long)ull;//ccteng GetTickCount();
}
...
#ifdef UNDER_CE
			SetWindowLong(hWnd, GWL_USERDATA, (LONG)(LONG_PTR)pCreate->lpCreateParams);
#else
...
*****************************************************************************/
int WINAPI WinMain0(HINSTANCE hInstance, HINSTANCE hPrevInstance, TCHAR *lpCmdLine, int nCmdShow)
{
	size_t			i;
	char			*pszCmdLine;
	PVRShellInit	init;
